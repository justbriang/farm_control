...
Widget entryField(String title,TextEditingController tcontroller, {bool isPassword = false}) {
  ...
  Widget editextfield(String title,String textContent, TextEditingController tcontroller, {bool isPassword = false}) {
    ValidatorClass validate = new ValidatorClass();
    return Container(
      margin: EdgeInsets.symmetric(vertical: 10),
      child: Column(
        ...
        TextFormField(
             validator: (value) => validate.checker(title, value??textContent),
              obscureText: isPassword,
